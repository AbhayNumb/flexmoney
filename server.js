...
function completePayment(
  ...
  pool.query(
    ...
    function (existingEnrollmentError, existingEnrollmentResult) {
      ...
      pool.query(userInsertQuery, userInsertValues, function (userError, userResults) {
        if (userError) {
        ...
        pool.query(paymentInsertQuery, paymentInsertValues, function (
          paymentError,
          paymentResults
        ) {
          if (paymentError) {
...
app.post("/enroll", (req, res) => {
  try {
    const { name, mobile_number, age, batch, month, year, paymentAmount } = req.body;
